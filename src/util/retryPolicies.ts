...
const getConfig = () => {
    return {
        ...
        exponentialRetryAfterMs: ifNumber(
            process.env.ExponentialRetryAfterMs,
            100
        ),
...
export class ExponentialBackOff extends DefaultRetryPolicy {
    constructor() {
        const config = getConfig();
        super(
            config.exponentialRetryMaxWaitTime,
            config.exponentialRetryAfterMs
        );
    }
